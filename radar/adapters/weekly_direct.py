"""Bounded official HTML/feed channels, selectively ported from PR #9."""
import re
import time
...
from radar.adapters.longtail import RssAdapter
...
class ChannelCoverage:
    def __init__(self,config):self.config=config;self.pages=0;self.discovered=0;self.complete=False;self.excluded=0;self.capped=False
    def report(self):
        return dict(scope='CONFIGURED_CHANNEL',pages_requested=self.pages,page_cap=self.config.get('max_pages',2),
                    ...
                    record_cap=self.config.get('max_records',30),record_cap_reached=self.capped,non_recruitment_excluded=self.excluded,
                    weekly_policy='GFC_WEEKLY_DIRECT_V1',
                    scope_note='First configured listing page or fixed recruitment section only; not institution-wide coverage')
...
class OfficialChannelAdapter:
    ...
    def discover(self):
        ...
        if method not in ('BOARD','RSS'):raise SourceFailure('NOT_CONFIGURED','Requested channel has no implemented adapter')
        ...
        for _ in range(max_pages):
            ...
            for candidate in candidates:
                ...
                if self.pagination.discovered>=limit:
                    self.pagination.capped=True;self.pagination.complete=True
                    return
    ...
    def fetch_detail(self,candidate):
        ...
        data,_,url=self.read_page(candidate.official_detail_url)
    ...
    def normalize(self,candidate,detail,documents):
        identity_keys=self.config.get('identity_query_keys')
        if identity_keys is not None:
            parts=urlsplit(detail.url)
            detail.url=urlunsplit((parts.scheme,parts.netloc,parts.path,
                urlencode([(k,v) for k,v in parse_qsl(parts.query) if k in identity_keys]),''))
        title=detail.title
        ...
        for label,key in [(r'행사\s*일시|개최\s*일시','event_start_at')]:
        ...
        candidate.raw_metadata.update(detail.raw_metadata)
        candidate.raw_metadata['weekly_direct_facts']=facts.model_dump()
        # Stable URL, new cohort/participation/region = a distinct recruitment.
        candidate.source_program_id=digest({'url':normalize_url(detail.url),
            'cohort':facts.cohort if fixed else None,'year':facts.year if fixed else None})
        return program
