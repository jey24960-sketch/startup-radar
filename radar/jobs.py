...
def dispatch_job(db,kind,source_slug=None,user_id=None):
    ...
    try:
        ...
        if response.status_code==204:
            with db.transaction() as c:current=c.execute('select state from radar.job_requests where id=%s',(job['id'],)).fetchone()
    ...
    with db.transaction() as c:
        changed=c.execute("update radar.job_requests set state=%s,result=%s,finished_at=now() where id=%s and state='REQUESTED' returning state",
            (state,Jsonb({'error':error}),job['id'])).fetchone()
        current=changed or c.execute('select state from radar.job_requests where id=%s',(job['id'],)).fetchone()
...
def cancel_job(db,job_id,note,user_id):
    ...
    with db.transaction() as c:
        ...
        job=c.execute('select * from radar.job_requests where id=%s for update',(job_id,)).fetchone()
        ...
        c.execute("update radar.job_requests set state='CANCELLED',finished_at=now(),result=%s where id=%s",
