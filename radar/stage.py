...
SIGNALS = {
    ...
    'STAGE_4': _rules([
        ...
        ('해외 진출·수출', r'해외\s*(진출|실증|파트너|법인|현지화|판로)|글로벌\s*(진출|실증|파트너|액셀러|스케일)|수출\s*(지원|기업|바우처|계약)|바이어\s*(매칭|상담|발굴)|현지\s*(진출|파트너|투자자)|국제\s*(전시|박람회)\s*(참가|참여)|통합관|파빌리온'),
    ]),
}
