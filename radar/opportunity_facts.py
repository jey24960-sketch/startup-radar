...
EXCLUDED_TITLE = re.compile(r'채용|조교\s*모집|서포터즈\s*모집|합격자|최종\s*선정|선정\s*결과|결과\s*발표|선발\s*결과|(?:수상|시상)\s*결과|수상자\s*(?:발표|안내)|투자\s*유치\s*(?:성공|완료)|보도자료|행사\s*후기|인턴\s*모집|경품|도록\s*발간')
...
def explicit_date(value, end=False):
    ...
    localized_clock=re.match(r'\s*(?:\([^)]{1,5}\))?\s*(오전|오후)\s*(\d{1,2}):([0-5]\d)',tail)
    korean_clock=re.match(r'\s*(?:\([^)]{1,5}\))?\s*(오전|오후)?\s*(\d{1,2})시(?:\s*([0-5]?\d)분)?',tail)
    try:
        hour=int(clock[1]) if clock else int(korean_clock[2]) if korean_clock else 23 if end else 0
        minute=int(clock[2]) if clock else int(korean_clock[3] or 0) if korean_clock else 59 if end else 0
        if localized_clock:
            hour=int(localized_clock[2]);minute=int(localized_clock[3])
            if not 1<=hour<=12:raise ValueError()
            hour=hour%12+(12 if localized_clock[1]=='오후' else 0)
        if korean_clock and korean_clock[1]:
            if not 1<=hour<=12:raise ValueError()
            hour=hour%12+(12 if korean_clock[1]=='오후' else 0)
        next_day=hour==24 and minute==0
        dt = datetime(int(match['y']),int(match['m']),int(match['d']),
                      0 if next_day else hour,minute,
                      0 if clock or korean_clock or localized_clock or not end else 59,tzinfo=SEOUL)
        if next_day:dt+=timedelta(days=1)
        return dt, 'DATETIME' if clock or korean_clock or localized_clock else 'DATE'
...
def labeled_period(text,context_year=None):
    ...
    match = re.search(r'(?:모집\s*기간|신청\s*기간|접수\s*기간|지원\s*기간|서류\s*접수|모집은)\s*[)）:：]?\s*', text) or re.search(r'모집\s*일정\s*[)）:：]?\s*',text)
    ...
    line=re.sub(r'^(?:은|는)\s*','',line)
    if context_year:
        line=re.sub(r'^(?:해당\s*연도|해당\s*년도)\s*',str(context_year)+'.',line)
    abbreviated=re.match(r"['’](\d{2})(?=[.년/-])",line)
    if abbreviated and context_year and int(abbreviated[1])==context_year%100:
        line=str(context_year)+line[abbreviated.end():]
    if re.match(r'[~∼～]',line):
        end,ep=explicit_date(line,True)
        return None,end,'UNKNOWN',ep,match[0]+line[:160]
    ...
    parts=re.split(r'\s*(?:[~∼～]|부터)\s*',line,maxsplit=1)
    ...
    if start and end is None and not re.search(r'20\d{2}',parts[1]):
        ...
        if short:end,ep=explicit_date(f'{start.year}.{short[1]}.{short[2]}{short[3]}',True)
            ...
            day=re.match(r'(\d{1,2})일(?:까지|에)',parts[1])
            if day:end,ep=explicit_date(f'{start.year}.{start.month}.{day[1]}',True)
...
def classify(title,text,config):
    ...
    for key,pattern in [('cohort',r'(?:배치\s*|batch\s*)?\d+\s*기(?![가-힣])|제\s*\d+\s*회'),('year',r'20\d{2}')]:
        match=re.search(pattern,title,re.I)
        if match:setattr(facts,key,int(match[0]) if key=='year' else re.sub(r'\s+','',match[0]));evidence(key,match[0])
    for label,field in [('참가비|참여비|비용','fee'),(r'투자\s*조건|지분\s*조건|지분율','investment_terms')]:
        match=re.search(r'(?:'+label+r')\s*[:：]\s*([^\n]{1,250})',text)
        if match:setattr(facts,field,match[1]);evidence(field,match[0])
    for label,key in [(r'지원\s*대상|신청\s*대상|신청\s*자격|모집\s*대상|참여\s*조건','target'),(r'지역\s*제한|소재지','regions'),(r'업종\s*제한|모집\s*분야','industries')]:
        match=re.search(r'(?:'+label+r')\s*[)）:：]?\s*([^\n]{2,250})',text)
        if match:
...
def weekly_decision(program, facts, at=None):
    """Only explicit current recruitment enters the automatic weekly article."""
    from radar.dates import program_status
    state = program_status(program, at)
    if facts.get('cancelled') or 'NOT_RECRUITMENT' in facts.get('review_reasons', []):
        return 'EXCLUDED', 'NOT_RECRUITMENT'
    if state in ('CLOSED', 'UPCOMING'):
        return 'EXCLUDED', state
    if not facts.get('recruitment_confirmed'):
        return 'REVIEW', 'RECRUITMENT_UNCONFIRMED'
    if state != 'OPEN':
        return 'REVIEW', 'CURRENT_PERIOD_UNKNOWN'
    if facts.get('participation') == 'UNKNOWN':
        return 'REVIEW', 'PARTICIPATION_UNKNOWN'
    if 'MULTIPLE_APPLICATION_LINKS' in facts.get('review_reasons', []):
        return 'REVIEW', 'MULTIPLE_APPLICATION_LINKS'
    return 'ACCEPTED', 'EXPLICIT_CURRENT_RECRUITMENT'
