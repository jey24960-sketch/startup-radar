...
## Evidence extraction 2.0.2

Unsupported complex OR/exceptions now make all requirements from that extraction uncertain, so a partial decomposition cannot create a false hard rejection. Independently supplied structured requirements remain present. A supported original hard requirement can still reject a team. Whitespace-only evidence cannot validate a rule.

Date extraction accepts optional ISO timestamps in `application_start_at`/`application_end_at` alongside legacy date fields. A full cited year/month/day and a matching explicit time are required for a timestamp. Date-only values are refused when their citation or nearby source context specifies a time. Existing structured dates have priority. No time in the source retains the documented date-only start/end-of-day convention. Unsupported abbreviated dates, missing years, non-adjacent time layouts and ambiguous context require review rather than guessed cutoffs. Literal validation is not a semantic accuracy guarantee.
