...
**Phase A is in progress; the final A/B/C Python server decision is not yet proven.** Four member read paths now use narrow Supabase RPCs: membership/team context, own-team profile, stored program feed and program detail. Deterministic Python batch refresh prepares five presets and current team results, including historical program versions. Team creation/profile updates now also use transactional RPCs with atomic history and calculation requests. Preferences and health remain conversion work. See [PROFILE-COMMANDS.md](PROFILE-COMMANDS.md). A residual Python call in the current code is an implementation task, not proof that Python hosting is necessary.
...
Persisted feed/detail and batch warming are implemented. Profile creation/update, immutable history and calculation requests are now atomic and verified for concurrency/rollback; next convert notification preferences and admin health. Scheduled/manual REFRESH processes version-specific requests while continuing cache warming. Apply each reviewed migration individually with a fresh shared-GFC inventory. Complete the A/B/C server decision after those contracts are implemented and verified, before considering hosting.
