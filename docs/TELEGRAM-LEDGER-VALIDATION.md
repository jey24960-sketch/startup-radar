...
4. In either phase, `finally` disables every subscription flag and restores the exact original team preferences. An `always()` cleanup step uses a same-run marker written only after preflight, so a terminated process can also restore the exact test scope. On total runner loss, inspect the marker/state and restore the validation team's original preferences before other operations. V2 schedules remain disabled throughout.
