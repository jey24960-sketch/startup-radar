...
**Telegram.** One message per weekly briefing to the one official GFC channel, counting published
...
**Normal weekly Telegram = ONE official GFC channel, not per-member or per-team subscriptions.**

The weekly briefing is the same curated publication for every member, so it is announced once,
to one official GFC Telegram channel that members join themselves. A member's whole onboarding
is: open the join link, join the channel, done. No Telegram ID, no chat ID, no team binding, no
operator step, no notification preference.

- Configuration lives in `startup_radar.runtime_settings` key `weekly_telegram_channel`
  (`enabled`, `chat_id`, `name`, `join_url`). `chat_id` is read only by the worker. Members see
  `enabled/name/join_url` through `public.gfc_radar_telegram_channel()`, a narrow SECURITY DEFINER
  RPC with the usual `my_role()` member/admin gate (the raw row is admin-only under RLS).
- `weekly.announce()` posts to that one chat. It never consults `telegram_subscriptions` or
  `team_notification_preferences`. Only `publication_kind='WEEKLY'` may broadcast; the baseline
  returns `BASELINE_ARCHIVE`. An unconfigured or disabled channel returns `NO_BROADCAST_CHANNEL`,
  a valid no-op that never fails the publication. `RADAR_V2_DELIVERY_ENABLED` remains the global
  gate (`DELIVERY_DISABLED` when off).
- The message is generated **only** from the published `weekly_briefing_items`, in display order:
  title, total `item_count`, at most three examples (GFC_RELEVANT sort first), and the member-only
  article link. Whatever the website withheld cannot appear. A zero-item week sends the concise
  notice "이번 주 새로 확인된 지원사업이 없습니다." with the article link — one message per
  published weekly issue, consistent with the existing publication contract.
- Ledger: `weekly_announcements` with `target_kind='OFFICIAL_CHANNEL'` and a null
  `subscription_id`; `unique(briefing_id, chat_id)` guarantees at most one post per issue. A short
  committed `SENDING` claim precedes the network call; `FAILED`/`UNCERTAIN`/`SENDING` rows are never
  automatically resent.

Legacy per-team subscriptions, notification preferences, HIGH_FIT/REMINDER batches and the admin
bot commands are preserved for advanced use and history; they are simply no longer on the weekly
delivery path. `/id` is not part of member onboarding.

### One-time operator setup

```text
1. Create the Telegram channel (e.g. "GFC StartupRadar") and make it public or create an invite link.
2. Add the existing StartupRadar bot as a channel administrator with "Post messages" permission.
3. python -m radar.cli telegram-channel --chat-id -100XXXXXXXXXX --join-url https://t.me/... --note "Initial channel setup"
4. TELEGRAM_BOT_TOKEN=... python -m radar.cli telegram-channel --verify      # read-only: token, channel access, admin/post permission
5. python -m radar.cli telegram-channel --enable --note "Verified; enabling official weekly channel"
6. (optional) TELEGRAM_BOT_TOKEN=... python -m radar.cli telegram-channel --test-post   # one clearly labelled test message
```
