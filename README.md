...
> **Latest: GFC /notice integration.** Member UI now belongs to the existing GFC React website. This repository provides ingestion, authenticated APIs and deterministic eligibility/recommendation services. Shared DB integration is applied; Phase A conversion is complete; real operational verification remains in progress. Production cutover is pending. Read the [integration report, contracts, access matrix and remaining setup](docs/GFC-NOTICE-INTEGRATION.md). The standalone dashboard is disabled by default. V1's scheduled workflow was observed `disabled_inactivity`; no workflow was re-enabled by this change.
