...
현재 주간 수집 범위는 두 공식 API의 제한된 최신 공고와 검증된 대학·민간·생태계 공식 채널 8곳입니다. 여러 출처를 수집해도 주간 공지와 Telegram 공지는 주당 하나로 유지합니다. Telegram 공지는 공식 GFC 채널 하나에 게시되며, 회원은 채널에 한 번 참여하면 됩니다(ID 등록·팀 연결 불필요). 수집은 넓게 하되 발행은 선별합니다. 신청 마감까지 최소 48시간이 남고 GFC 창업팀에게 해당하는 공고만 회원 화면에 표시하며, 나머지는 삭제하지 않고 감사 가능하게 보관합니다. 회원 화면의 "새로 확인"은 StartupRadar가 처음 확인했다는 뜻이며, 기관이 그 주에 공고했다는 의미가 아닙니다. [정확한 출처·범위·제외 기준](docs/WEEKLY-BRIEFING.md)을 참고하세요. 전국·과거 공고 전수 수집을 보장하지 않으며, 직접 수집 공고의 모집 시점이 불명확하면 자동 발행에서 제외합니다. 추천은 선정 또는 최종 신청 자격을 보장하지 않습니다.
