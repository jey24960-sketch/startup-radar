> **Current operating goal:** Continue from existing code toward real GFC operation. A standalone Python API host is **not required by default**. Read the [controlling goal](docs/GFC-OPERATING-GOAL.md) and [Phase A endpoint decisions](docs/PYTHON-API-DECISION.md). Team context/profile, stored feed and detail reads now use Supabase RPC with existing RLS; team creation and profile saves now use transactional RPCs with history/calculation requests. Preferences and admin health are being converted; see [profile command contract](docs/PROFILE-COMMANDS.md). Preserve the batch engine and keep V2 schedules disabled pending live validation.
